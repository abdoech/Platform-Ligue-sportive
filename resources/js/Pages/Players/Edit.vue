<template>
    <Head :title="'Modifier ' + player.full_name" />

    <div class="min-h-screen bg-gray-100">
        <div class="max-w-3xl mx-auto py-6 sm:px-6 lg:px-8">
            <div class="px-4 py-6 sm:px-0">
                <div class="mb-6">
                    <a :href="`/admin/players/${player.id}`" class="text-sm text-gray-500 hover:text-gray-700">← Retour au joueur</a>
                </div>

                <h1 class="text-3xl font-bold text-gray-900 mb-6">Modifier {{ player.full_name }}</h1>

                <form @submit.prevent="form.put(`/admin/players/${player.id}`)" class="bg-white shadow rounded-lg p-6 space-y-4">
                    <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input id="position" v-model="form.position" type="text" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2" />
                        </div>
                        <div>
                    ...
                    <div class="flex gap-3 pt-4">
                        ...
                        <a :href="`/admin/players/${player.id}`" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium py-2 px-4 rounded">Annuler</a>
                    </div>
                </form>
            </div>
        </div>
...
const form = useForm({
    ...
    date_of_birth: props.player.date_of_birth ? props.player.date_of_birth.split('T')[0] : '',
